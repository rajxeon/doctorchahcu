...
<div id="page-wrapper" style=" position:relative;min-height: 100px;"  class="right-side">
  <div class="col-xs-12 page-header">
    <div class="col-xs-2 no-pad"> <a data-toggle="modal" onclick="event.preventDefault();" class="btn btn-default" href="<?php echo bu('doctor/records/0/completed_procedure')?>" data-target=".bs-example-modal-lg"> <i class="fa fa-fw fa-users"></i> </a>
      <h4 style="display: inline;">All Paatients </h4>
    </div>
    <div class="col-xs-7">
      <a href="<?php echo bu('doctor/view_patient/'.$patient_data->id);?>">
          <div class="col-md-4 col-xs-12 no-pad" style="margin-top: -6px;text-overflow: ellipsis;white-space: nowrap;">
            <?php 
            $img_src='';
    ...
    onclick="event.preventDefault();prepare_json($(this))"><i class="fa fa-fw fa-save"></i> Update </a> 
    <a href="<?php if(isset($_SERVER['HTTP_REFERER'])) echo $_SERVER['HTTP_REFERER'];
	else echo bu('doctor/completed_procedure'); ?>"  class="btn btn-sm btn-flat btn-default fr"  style="width:100px"><i class="fa fa-fw fa-times"></i> Cancel </a> </div>
  </div>
  <!-- End of header--> 
...
$(document).ready(function(e) {
	...
	for(var i in json){  
		...
		if(json[i].invoiced==1)
		dummy+='<input type="hidden" class="invoiced" value="1">';
		
		dummy+='<input type="hidden" class="speciality" value="'+json[i].speciality+'">';
		
		if(json[i].invoiced==0)
		dummy+='<i class="fa fa-fw fa-times-circle close" onClick="$(this).parent().remove();calculate_gross()"></i>';
		
		dummy+='<div class="col-xs-4 no-pad"><p><u class="treatment_plan_name">'+json[i].treatment_plan_name+'</u></p><a href=""';
		...
		if(json[i].invoiced==1) dummy+='disabled="disabled" ';
...
function prepare_json(self){
	...
	$('.tabber').each(function(index, element) {
		...
		if($(this).find('.invoiced').length>0) 
		invoiced	=1;
		else
		invoiced	=0;
		multiply=0;
		...
		'total_price':total_price,'note_txt':note_txt,'vig':vig,'vig_txt':vig_txt,'multiply':multiply,'select_all_teeth':select_all_teeth,'completed':completed,'invoiced':invoiced,'treatment_ids':1
		};
		
		array[index]=object;
    });
	$.post('<?php echo bu('doctor/ajax_edit_completed_procedure');?>',{
		t_id:'<?php echo $t_data->id;?>',
		doctor_id:doctor_id,
		clinic_id:clinic_id, 
		...
		json:JSON.stringify(array)
			...
			window.location.href='<?php echo bu('doctor/completed_procedure/'.$patient_data->id);?>';
			}
