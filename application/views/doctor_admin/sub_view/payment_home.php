...
td>a{ color:#3c8dbc !important}
.p_parent table{ width:100%}
...
$(document).ready(function(e) { 
    $.post('<?php echo bu('doctor/get_payment/0');?>',{<?php if(isset($pid)) echo 'pid:'.$pid?>},function(data){ 
		$('#treatment_plan_tile_holder').html(data);
		});
});

function append_result(offset){
	$.post('<?php echo bu('doctor/get_payment/');?>'+offset,{<?php if(isset($pid))  echo 'pid:'.$pid?>},function(data){ 
	...
	var myJsonString = JSON.stringify(delta_array); 
 	...
 	$.post('<?php echo bu('doctor/invoice_selected');?>',{json:myJsonString},function(data){
		...
		if(data==1){
			$('.ajax-marker').hide(0);
			$('.chk-marker').show(0);
			window.location.href="<?php echo refferer('doctor/completed_procedure/'.@$pid);?>";
			}
		});	
	}
...
<h1 class="page-header" style="padding: 10px;">
	<?php if(isset($pid)): ?>
	  <a href="<?php echo bu('doctor/refund_invoice/'.$pid);?>" class="btn btn-success btn-flat btn-sm pull-right"  >
      <i class="fa fa-dollar "  ></i>
      Refund</a>
    <?php endif;?>
    <a data-toggle="modal" onclick="event.preventDefault();" class="btn btn-default" href="<?php echo bu('doctor/records/0/payment')?>" data-target=".bs-example-modal-lg">
     	 <i class="fa fa-fw fa-users"></i>
     </a>
     <?php if(isset($pid)):?>
      <a style="margin:0 5px" href="<?php echo bu('doctor/add_payment/'.$pid);?>" class="btn btn-primary btn-flat btn-sm pull-right">+ Add</a>
      <?php endif;?>
    PAYMENTS  
</h1>
