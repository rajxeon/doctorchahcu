...
$(document).ready(function(e) { 
    $.post('<?php echo bu('doctor/get_prescription/0');?>',{<?php if(isset($pid)) echo 'pid:'.$pid?>},function(data){ 
		$('#treatment_plan_tile_holder').html(data);
		});
});

function append_result(offset){
	$.post('<?php echo bu('doctor/get_prescription/');?>'+offset,{<?php if(isset($pid))  echo 'pid:'.$pid?>},function(data){ 
...
<h1 class="page-header" style="padding: 10px;">
    ...
    <a data-toggle="modal" onclick="event.preventDefault();" class="btn btn-default" href="<?php echo bu('doctor/records/0/prescription')?>" data-target=".bs-example-modal-lg">
     	 <i class="fa fa-fw fa-users"></i>
     </a>
     <?php if(isset($pid)):?>
      <a style="margin:0 5px" href="<?php echo bu('doctor/add_prescription/'.$pid);?>" class="btn btn-primary btn-flat btn-sm pull-right">+ Add New</a>
      <?php endif;?>
    All Prescriptions
</h1>
