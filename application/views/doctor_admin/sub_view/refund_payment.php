...
<script>
function calculate_after_refund(){ 
	after_refund= $('#advance_payment').text()-$('.amount').val(); 
...
function prepare_json(self){
	...
	if(flag=='refund'){
		...
		$.post('<?php echo bu('doctor/ajax_add_refund');?>',{  
	...
	if(flag=='inv_pay'){
		...
		$.post('<?php echo bu('doctor/ajax_refund_by_invoice');?>',{  
			patient_id:patient_id, 
			clinic_id:clinic_id,
			note:$('.note').val(),
			mode:$('#inv_mode').val(),
...
function calculate_after_pay(){
	...
	$('.cpl').each(function(index, element) {
		...
		parent.find('.dap').text(due_h-paid_h+parseInt($(this).val()));
...
$('.inv_selected').each(function(index, element) { 
	...
	html+='<tr data-index_id_f="'+$(this).attr('data-index_id')+'" class="due_h">';
	 ...
	 html+='</td>';
	  ...
	  html+='<td>';
		html+= '<input type="number" max="'+parseInt($(this).attr('data-paid'))+'" class="form-group cpl" pattern="[0-9]*"  value="'+parseInt($(this).attr('data-paid'))+'" >' ;
	 html+='</td>';
...
?>
<input type="hidden" id="helper_flag" value="refund" />
  <div class="col-sm-8 col-xs-12 no-pad"> 
    <div class="nav-tabs-custom">
      <ul class="nav nav-tabs  ">
        <li class=" header"><i class="fa fa-money"></i> ReFund</li> 
        <a href="" class="btn btn-default btn-sm pull-right disabled btn-flat" style="margin:5px">Refund</a>
        <a href="<?php echo bu('doctor/add_payment/'.$patient_data->id);?>" class="btn btn-flat btn-success btn-sm pull-right" style="margin:5px">Add Fund</a>
      </ul>
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane active" id="tab_1-1" style="height:67vh; overflow:auto; overflow-x:hidden">
       	...
       	<div class="col-xs-12 payment_only" style="margin: 10px 0;font-size: 19px;border-bottom: 1px dashed;border-top: 1px dashed;">
            TOTAL AVAILABLE BALENCE: <span id="advance_payment"><?=(@$payment_data->balence);?></span>
            <?php if(@$payment_data->balence>0){ echo ' <small class="badge bg-green">(Advance)</small>';}
            if(@$payment_data->balence<0){ echo ' <small class="badge bg-red">(Due)</small>';}
            ?>
        </div>
        <div class="box box-solid bg-yellow payment_only">
            <div class="box-header" style="color: #000;">
                ...
                <input type="number" class="form-control amount" value="0" min="0" onkeyup="calculate_after_refund()" onchange="calculate_after_refund()"></div>
                <div class="col-xs-2 no-pad">
                ...
                <div class="col-xs-5 ">
                <small >*This will deducted from balence</small>
               </div>
                </h3>
            </div>
            <div class="box-body">
        ...
        <div class="col-xs-12 payment_only" style="margin: 10px 0;font-size: 15px;border-bottom: 1px dashed;border-top: 1px dashed;">
        ADVANCE AFTER REFUND: <span id="after_refund"></span>
        <div class="callout callout-danger" id="warner" style="display:none"> <h4>Refund Amount is greater than 
        <i class="fa fa-inr"></i>
        <strong><?=(@$payment_data->balence);?></strong></h4>  </div>
        </div>
        ...
        <table class="table table-bordered" style="margin-bottom:5px">
        	<thead>
            	<tr>
                    ...
                    <th>REFUND (<i class="fa fa-inr"></i>)</th>
                    <th>DUE AFTER REFUND (<i class="fa fa-inr"></i>)</th>
                    <th></th>
                </tr>
        ...
        <div class="row">
            ...
            <div style="background:#FFFAC3;padding: 5px; margin:0" class="doted-border row"> 
            	<div class="col-xs-5">TOTAL REFUND:</div> 
        ...
        <div class="taber_header col-xs-12 no-pad">
          <div class="col-xs-4 no-pad">TREATMENT</div>
           ...
           <button id="sacrificer" class="btn btn-default btn-flat disabled" style="margin:80px auto; display:block">
            	You can also select paid invoice(s) from the right
            </button>
           </div>
  ...
  <div class="col-sm-4 col-xs-12 no-pad">
    <div class="box" style="height:81vh; ">
        <div class="box-header" data-toggle="tooltip" title="" data-original-title="Procedures ( in INR )">
            <h3 class="box-title">Paid Invoice</h3>
