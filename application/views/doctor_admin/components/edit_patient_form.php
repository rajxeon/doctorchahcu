...
$(document).ready(function(e) {
	...
	$('#meeo_c').click(function(e) {
	 ...
	 //get_medical_history();
	 //get_group_history();
	 $.when(populate_referer(), get_medical_history(), get_group_history()).done(function(){ 
		 find_mobile($('#p_phone'));
...
function find_mobile(self){
	...
	if(val.length>=10){
		...
		if(!isNaN(val)){
			clear_data(0);
			$.post('<?php echo bu('doctor/get_p_details_by_id/')?>',{pid:<?php echo $p_data->id;?>},function(data){
				data=data.trim();
				if(data==='empty'){
...
function save_patient_details(){
	...
	$.post('<?php echo bu('doctor/edit_patient_handler/'.$p_data->id)?>',{
		p_name:p_name,
		p_aadhaar:p_aadhaar,
		...
		window.location.href="<?php echo bu('doctor/view_patient/'.$p_data->id);?>";
		}); 
	}
...
<div class="input-group"> <span class="input-group-addon high_same"> Mobile <span class="high">*</span></span>
  <input type="text"  pattern="[0-9]*"  class="form-control" placeholder="Ex:9475956718" onKeyUp="find_mobile($(this))"  id="p_phone" value="<?php echo $p_data->phone;?>" readonly="readonly">
</div>
<div class="input-group"> <span class="input-group-addon high_same">Patient Name <span class="high">*</span></span>
  <input type="text" class="form-control ini_inactive" disabled placeholder="Patient Name"  id="p_name">
</div>
